...
public class TenantAppService : AsyncCrudAppService<Tenant, TenantDto, int, PagedTenantResultRequestDto, CreateTenantDto, TenantDto>, ITenantAppService
{
    ...
    public override async Task<TenantDto> CreateAsync(CreateTenantDto input)
    {
        ...
        {
            tenant.EditionId = input.EditionId.Value;
        }
        else
        {
    ...
    protected override void MapToEntity(TenantDto updateInput, Tenant entity)
    {
        ...
        entity.EditionId = updateInput.EditionId;
    }

    public async Task<List<SubscribableEditionComboboxItemDto>> GetEditionsForCombobox()
    {
        var editions = await _editionManager.Editions.ToListAsync();
        return editions
            .Select(e => new SubscribableEditionComboboxItemDto(e.Id.ToString(), e.DisplayName, false))
            .ToList();
    }
    ...
    public async Task<List<TenantUserLookupDto>> GetTenantUsers(EntityDto input)
    {
        using (CurrentUnitOfWork.SetTenantId(input.Id))
        {
            var users = await _userManager.Users
                .Where(u => u.TenantId == input.Id && u.IsActive)
                .Select(u => new TenantUserLookupDto
                {
                    Id = u.Id,
                    UserName = u.UserName,
                    Name = u.Name,
